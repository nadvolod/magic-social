...
class LearningState:
    ...
    archived_not_published_reasons: dict = field(default_factory=dict)  # archived reasons (served their purpose)
    total_feedback_received: int = 0
    total_ratings_received: int = 0
    average_rating: float = 0.0
    explicit_ratings_received: int = 0   # ratings from real user feedback only
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "archived_not_published_reasons": self.archived_not_published_reasons,
            "total_feedback_received": self.total_feedback_received,
            "total_ratings_received": self.total_ratings_received,
            "average_rating": self.average_rating,
            "explicit_ratings_received": self.explicit_ratings_received,
    ...
    def from_dict(cls, data: dict) -> "LearningState":
        return cls(
            ...
            archived_not_published_reasons=data.get("archived_not_published_reasons", {}),
            ...
            explicit_ratings_received=data.get("explicit_ratings_received", 0),
...
# Implicit/automated reason keys generated by infer_implicit_feedback
_IMPLICIT_FEEDBACK_REASONS = {"no_feedback_72h", "stale_unpublished_7d"}
...
def _apply_qualitative_feedback(state: LearningState, feedback: PostFeedback) -> None:
    ...
    - Separate explicit (user) vs implicit (automated) rating averages
    """
    ...
    is_implicit = reason in _IMPLICIT_FEEDBACK_REASONS
    ...
    if feedback.rating is not None:
        ...
        # Track explicit (user-provided) ratings separately
        if not is_implicit:
            prev_explicit = state.explicit_ratings_received
            state.explicit_ratings_received += 1
            state.explicit_average_rating = (
                (state.explicit_average_rating * prev_explicit + feedback.rating)
                / state.explicit_ratings_received
            )

    logger.info(
        "Qualitative feedback applied: published=%s, rating=%s, implicit=%s, reasons=%s",
        feedback.published,
        feedback.rating,
        is_implicit,
